...
if __name__ == '__main__':
    ...
    ---res
       ...
       ---DSM_Result
          ...
          ...
    '''
    path = '/workspace/DFC2023/test_demo/'
    gt_path = os.path.join(path, 'ref')
    pred_path = os.path.join(path, 'res')
    Output_path = os.path.join(path, 'output')

    seg_gt_path = os.path.join(gt_path, 'gt.json')
    dsm_gt_path = os.path.join(gt_path, 'gt_dsm')
    dsm_gt_list = os.listdir(dsm_gt_path)

    seg_pred_path = os.path.join(pred_path, 'results.segm.json')
    dsm_pred_path = os.path.join(pred_path, 'DSMResult')
    dsm_pred_list = os.listdir(dsm_pred_path)

    output_file = os.path.join(Output_path, 'metric.txt')
    ...
    for dsm_pred_i in dsm_pred_list:
        ...
        dsm_pred_tif = TIFF.open(dsm_pred_path_i, mode='r')
        dsm_gt_tif = TIFF.open(dsm_gt_path_i, mode='r')
        dsm_pred_array = dsm_pred_tif.read_image()
        dsm_gt_array = dsm_gt_tif.read_image()
    ...
    with open(output_file, 'w') as out:
        ...
        out.write('mAP50' + ':' + str(mAP50)[:5] + '\n')
        out.write('MAE' + ':' + str(MAE)[:5] + '\n')
        out.write('RMSE' + ':' + str(RMSE)[:5] + '\n')
        out.write('Delta1' + ':' + str(Delta1)[:6] + '\n')
        out.write('Delta2' + ':' + str(Delta2)[:6] + '\n')
        out.write('Delta3' + ':' + str(Delta3)[:6] + '\n')
        out.close()
